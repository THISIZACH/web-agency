...
export function ContactSection() {
  ...
  const validate = () => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = locale === 'pt' ? 'Por favor insira o seu nome' : locale === 'ar' ? 'يرجى إدخال اسمك' : locale === 'fr' ? 'Veuillez entrer votre nom' : 'Name is required';
    }
    if (!formData.businessName.trim()) {
      newErrors.businessName = locale === 'pt' ? 'Nome da empresa é obrigatório' : locale === 'ar' ? 'اسم النشاط التجاري مطلوب' : locale === 'fr' ? 'Nom de votre entreprise requis' : 'Business name is required';
    }
    if (!formData.email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
      newErrors.email = locale === 'pt' ? 'E-mail profissional válido é obrigatório' : locale === 'ar' ? 'يرجى إدخال بريد إلكتروني صحيح' : locale === 'fr' ? 'Un e-mail professionnel valide est requis' : 'Valid email is required';
    }
    if (!formData.phone.trim()) {
      newErrors.phone = locale === 'pt' ? 'Contacto telefónico é obrigatório' : locale === 'ar' ? 'رقم الهاتف مطلوب' : locale === 'fr' ? 'Numéro de téléphone requis' : 'Phone number is required';
    }
    setErrors(newErrors);
    ...
    try {
      ...
      setErrorMessage(
        locale === 'pt'
          ...
          : locale === 'fr'
          ? 'Erreur réseau ou de connexion. Veuillez réessayer ou nous contacter via WhatsApp.'
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {copiedCard ? (
                    <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-600 dark:text-emerald-400 animate-in fade-in duration-150">
                      ...
                      <span>{locale === 'pt' ? 'Copiado!' : locale === 'ar' ? 'تم النسخ!' : locale === 'fr' ? 'Copié !' : 'Copied!'}</span>
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-xs font-medium text-slate-500 dark:text-slate-400 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">
                      ...
                      <span className="hidden sm:inline">{locale === 'pt' ? 'Copiar' : locale === 'ar' ? 'نسخ' : locale === 'fr' ? 'Copier' : 'Copy'}</span>
                    </span>
                  )}
                </div>
              </button>
                ...
                <div className="p-6 sm:p-8 md:p-10 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-center space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-300 max-w-md mx-auto leading-relaxed">
                      {locale === 'pt'
                        ...
                        : locale === 'fr'
                        ? 'Message bien reçu ! Pour obtenir une réponse immédiate de notre équipe, poursuivez via WhatsApp ou par e-mail.'
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
                    <a
                      href={`https://wa.me/${CONTACT_CONFIG.whatsappNumber}?text=${encodeURIComponent(
                        locale === 'pt'
                          ...
                          : locale === 'fr'
                          ? "Bonjour NexaWeb Studio ! Je viens d'envoyer une demande de projet sur votre site web."
                    ...
                    >
                      ...
                      <span>{locale === 'pt' ? 'Continuar no WhatsApp' : locale === 'ar' ? 'المتابعة عبر واتساب' : locale === 'fr' ? 'Continuer sur WhatsApp' : 'Continue via WhatsApp'}</span>
                    </a>
                    <button
                      type="button"
                  ...
                  <div className="pt-2">
                    ...
                    >
                      {locale === 'pt' ? 'Enviar outro pedido' : locale === 'ar' ? 'إرسال استفسار آخر' : locale === 'fr' ? 'Envoyer une autre demande' : 'Submit another request'}
                    </button>
                  </div>
                </div>
              ) : (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all"
                      >
                        <option value="">{locale === 'pt' ? 'Selecione uma opção' : locale === 'ar' ? 'اختر خياراً' : locale === 'fr' ? 'Sélectionnez une option' : 'Select an option'}</option>
                  ...
                  {status === 'error' && errorMessage && (
                    <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-600 dark:text-red-400 text-sm flex items-start gap-3 animate-in fade-in duration-200">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold">{locale === 'pt' ? 'Não foi possível enviar' : locale === 'ar' ? 'تعذر الإرسال' : locale === 'fr' ? 'Échec de l’envoi' : 'Submission failed'}</p>
